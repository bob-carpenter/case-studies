...
MiniStan only has sampling statements, so that the set
...
function $[\![p]\!]: \mathbb{R}^M \rightarrow \mathbb{R}^N \rightarrow
\mathbb{R}$.  Given $y \in \mathbb{R}^M$ and $\theta \in
\mathbb{R}^N$, define an environment $\Gamma$ such that
$$
\Gamma_{y,\theta}(v)
=
\begin{cases}
...
Given this environment, the denotation of a Stan program may be
defined by
$$
[\![p]\!]
(y)(\theta)
= \sum_{j = 1}^J {}_{\Gamma_{y, \theta}}[\![ f(e_{j,0}, e_{j,1}, \ldots, e_{j,K_j}) ]\!]
$$
where $s_j = e_{j, 0} \sim f(e_{j, 1}, \ldots, e_{j, K_j});$.  All
density functions are assumed to be on the log scale so that addition
is the appropriate combination.

In Bayesian applications, the statements are defined so that $y$ is
data, $\theta$ are model parameters, and the denotation is equal to
the log posterior up to a constant that does not depend on $\theta$,

$$
[\![p]\!]
(d)(u)
=
\log p(\theta \mid y) + \textrm{const.}
$$
