...
This short technical note evaluates several approaches to training a
logistic regression classifier from a probabilistic corpus in which
the probabilities of each item's outcome are known either exactly or
approximately.  The motivating example is the probability estimated by
a model of data rating (also known as coding, annotation, labeling,
and crowdsourcing), such as that of @dawid1979.
...
First, the logistic regression data generating process is presented.

In the next section, we simulate data according to the five schemes we
will be evaluating.
...
data-generating process for the covariates.  The reason for this is
that the data-generating process for outcomes is independent of the
data-generating process for covariates, allowing us to restrict
attention to simulating the outcomes.  It is always possible to
conjoin a data-generating process for the covariates---there is an
...
N = 50       # number of data points used to train
rho = 0.9      # correlation of predictor RW covariance
N_test = 20  # number of test items
M = 5         # number of simulation runs
```
...
pandas one-liner.  Here, higher ELPD is better.
