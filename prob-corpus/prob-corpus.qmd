...
These results hold for both Bayesian posterior mean estimators and
...
$x_n \in \mathbb{R}^D$ and $y_n \in \{ 0, 1 \}$ for $n \in 1{:}N.$.
Now suppose there are new items with covariates $\widetilde{x}_n$ for
$n \in 1{:}\widetilde{N}.$  For each new item $\widetilde{x}_n,$ the
prediction for an outcome $\widetilde{y}_n \in \{ 0, 1 \}$ is given by
the posterior predictive probability mass function,
\begin{align*}
p(\widetilde{y} \mid \widetilde{x}_n, x, y)
&= \mathbb{E}\!\left[\widetilde{Y} \,\big|\, \widetilde{x}_n, x, y\right]
\\[4pt]
&= \int_{\mathbb{R}^D} p(\widetilde{y} \mid \beta) \cdot p(\beta \mid
x, y) \ \textrm{d}\beta.
\end{align*}
...
p(\widetilde{y} \mid \widetilde{x}_n, x, y)
\approx
\frac{1}{M} \sum_{m = 1}^M p\!\left(\widetilde{y}_n \,\big|\, \widetilde{x}_n, 
\beta^{(m)}\right).
$$
...
p(\widetilde{y} \mid \widetilde{x}_n, x, y)
\approx
p(\widetilde{y} \mid \widetilde{x}_n, \beta^*).
$$
...
```{python}
D = 11        # number of predictors including intercept
N = 100       # number of data points used to train
rho = 0.9     # correlation of predictor RW covariance
N_test = 100  # number of test items
...
summary = df.groupby(['estimator','data'])['error'].apply(summary).unstack()
...
for m in range(M):
    ...
    Pr_rate_max = np.exp(lp_mle_max / N_test)
    Pr_rate_random = np.exp(lp_mle_random / N_test)
    Pr_rate_probs = np.exp(lp_mle_probs / N_test)
    Pr_rate_weights = np.exp(lp_mle_weights / N_test)
    Pr_rate_noisy_weights = np.exp(lp_mle_noisy_weights / N_test)

#    print(f"{lp_mle_max=}  {lp_mle_random=}  {lp_mle_probs=}  {lp_mle_weights=}  {lp_mle_noisy_weights=}")
#    print(f"{Pr_rate_max=}  {Pr_rate_random=}  {Pr_rate_probs=}  {Pr_rate_weights=}  {Pr_rate_noisy_weights=}")
    ...
    elpd_max = predict(test_data, fit_max, model_predict)
    elpd_random = predict(test_data, fit_random, model_predict)
    elpd_probs = predict(test_data, fit_probs, model_predict)
    elpd_weights = predict(test_data, fit_weights, model_predict)
    elpd_noisy_weights = predict(test_data, fit_noisy_weights, model_predict)
    print(f"{elpd_max=}  {elpd_random=}  {elpd_probs=}  {elpd_weights=}  {elpd_noisy_weights=}")
```
...
Skene's model to a Bayesian hierarchical model as shown in the "Latent
discrete parameters" chapter of the *Stan User's Guide.*

@raykar2010 is the first example of jointly training a classifier and
crowdsourcing model.  The maximum likelihood approach the authors
employ can be easily extended to a Bayesian hierarchical model.

@passonneau2014 use a million crowdsourced ratings over forty thousand
items for word sense disambiguation in natural language processing.
This paper provides a high-level motivation for rating models over
weighted voting approaches and inter-annotator agreement measures.
...
the ratings of some items did not converge, which further undercuts
