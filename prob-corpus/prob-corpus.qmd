...
def fit_bayes(model, data_dict):
    s = model.sample(data = data_dict, show_progress = False,
...
Logistic regression is popular because after a model is trained, it
may be used predictively to map a vector of covariates for a new item
to a probabilistic prediction of its category.
...
&= \mathbb{E}[\widetilde{Y} \mid  \widetilde{x}_n, x, y)
\\
&= \int_{\mathbb{R}^D} p(\widetilde{y} \mid \beta) \cdot p(\beta \mid
x, y) \ \textrm{d}\beta.
\end{align*}
With MCMC draws $\draw{\beta}{m} \sim p(\beta \mid x, y)$ for $m \in 1{:}M,$this quantity can be estimated as
$$
p(\widetilde{y} \mid \widetilde{x}_n, x, y)
\approx
\frac{1}{M} \sum_{m = 1}^M p(\widetilde{y}_n \mid \widetilde{x}_n \mid
\beta).
$$
...
likelihood estimate $\beta^* = \textrm{arg max}_\beta \log p(y \mid x,
\beta) - \mathcal{P}(\beta),$ and plugging that in for prediction,
$$
p(\widetilde{y} \mid \widetilde{x}_n, x, y)
\approx
p(\widetilde{y} \mid \beta^*).
$$
...
@stan2023 describes the Stan language and @stan2023 provides a user
manual with descriptions of how to fit both crowdsourcing and logistic
regression models.
