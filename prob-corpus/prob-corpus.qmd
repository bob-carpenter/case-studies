...
majority vote approach are instances of this suboptimal strategy.

The second result is that it is even better to use the probabilistic
data directly.  The second result is a direct consequence of the
Rao-Blackwell theorem in the Bayesian setting.

Evaluations use both Bayesian posterior mean estimators and penalized
maximum likelihood estimators.  As expected from theory, Bayesian
posterior means show lower squared error versus the true parameter
values.
