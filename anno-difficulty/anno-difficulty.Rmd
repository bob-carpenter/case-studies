---
title: "Discrete rating models with accuracy, bias, difficulty, discrimination, and guessing"
...
Data rating problems go by many names and have many applications in
many literatures.  They are known as item-response models in
educational testing when the items are test questions and the raters
are students taking the test.  In dignostic testing, the items are
subjects and the raters are diagnostic tests such as blood tests,
physical exams, or X-rays.  In ensemble machine learning, the raters
are machine learning classifiers and the items are anything being
classified, such as images of street scenes being classified for the
existence of pedestrians.  In machine learning test corpus creation,
the raters are the corpus annotators and the items are what is being
classified, such as words in natural language being classified for
grammatical part of speech.  In statistics, all of these models are
just noisy measurement models.

This paper surveys approaches used primarily for data annotation in
machine-learning or statistical estimation problems.  The simplest
model is based on voting.  The raters are all assumed to have the same
accuracy (that is, they are completely pooled).  This accuracy must be
estimated along with the gold standard and prevalence of positive
items.  Often voting models are simplified by assuming uniform
prevalence of items, as is often found in artificially constructed
data sets.  When such models are quantified, they produce a
voting-based approach to producing a gold standard.  This simple model
provides estimates of prevalence, aggregate rater accuracy, and the
status of each item.

A more complex model assumes that each rater has a different
accuracy (no pooling).  The resulting gold-standard inferernces
correspond to weighted voting, where the raters' votes are weighted by
their estimated probability of making a mistake.  This model provides
estimates of prevalence, individual rater accuracy, and the status of
each item.

Each rater's accuracy can be factored into a sensitivity (accuracy on
positive items) and specificity (accuracy on negative items).  This
accounts for rater bias (favoring positive or negative answers) as
well as asymmetric difficulty in problems such as information
retrieval where there may be few, hard-to-find positive items and a
large body of easy-to-classify negative items. The sensitivity and
specificity model allows the ratings of biased raters to be more
deconvolved in estimating the gold standard.  This is especially
important in multivariate ordinal tasks, such as ratings on a 1--5
scale.  Theoretically, at least, this allows adversarial raters to be
useful as long as they have positive mutual information.  It also
allows the classification of pure spammers, who show up in
crowdsourcing applications.  A spammer's response is independent of
the item being rated, and thus their sensitivity is equal to one minus
their specificity.  This model provides estimates of prevalence,
individual rater sensitivity and specificity, and the status of each
item.  

Each item may have a difficulty associated with it, making raters more
or less accurate when rating it.  This accounts for the natural
occurrence in data sets of easy and hard to rate items.  Item
difficulty is necessary to get a good fit of the model to real data
sets, all of which are overdispersed compared to what would be
expected from the model without item difficulty.  This model was first
introduced by Qu, Tan, and Kutner (1996).

Adding just item-level difficulty without splitting accuracy into
sensitivity and specificity corresponds to a standard item-response
theory one-parameter logistic model (IRT-1PL).  Such models were
introduced by Lord (1950) in an educational testing setting, albeit
with known item statuses.

The voting model is standard operating procedure in machine learning
corpus creation, often with censoring (removing items that do not have
a majority vote) and/or randomization (assigning a random value when a
vote is tied).  Crowdsourced data has sometimes it is extended to
weighted voting. Prevalence, confusingly known as "bias" in machine
learning classification problems, can be ignored if the items are
balanced, with equal numbeers of each category.  Often, prevalence is
ignored even when the items are not balanced.

Historically, IRT-1PL models were introduced by Lord (1950), albeit
with known item status as the items were exam questions.  Dawid and
Skene (1979) introduced the model that splits accuracy into
sensitivity and specificity and applied it to a corpus of medical
records with doctors rating whether the chart indicated the patient
smoked or not.  Qu, Tan, and Kutner (1996) introducd the
item-difficulty model with sensitivity and specificity.  When items
are unknown, the expectation maximimzation algorithm has been used to
fit maximum marginal likelihood estimates.
...
$1 - \textrm{logit}^{-1}(\theta_{j, 0}) = \textrm{logit}^{-1}(-\theta_{j, 0}).$
...
```{r}
caries_data <- read.table('data/original/caries-data.R', header = TRUE, sep = ",")
...
```{r results = 'hide'}
model_logit <- stan_model('stan/dawid-skene-logit.stan')
```

Then we fit the model using the default no-U-turn sampler with 2
chains of 2000 total iterations, half devoted to warmup.
...
```{r echo = FALSE}
print_file('stan/dawid-skene-logit-diff.stan')
```
...
normal favors constrained probabilities near 0.5].
...
```{r results = 'hide'}
model_diff <- stan_model('stan/dawid-skene-logit-diff.stan')
```
...
fit_diff <- sampling(model_diff, data = list(I = I, J = 5, y = y),
                     seed = 1234,
                     init = 0,
                     iter = M_diff * 2, chains = 2, refresh = 10,
                     open_progress = FALSE)
```
...
The estimate^[As a Bayesian estimator, the posterior mean minimizes expected square error of the estimate.]
of log odds of a positive case are very similar to the
...
Code Copyright (2019) Simons Foundation.  The code is
