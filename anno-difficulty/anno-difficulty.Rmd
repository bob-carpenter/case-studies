...
degrees of freedom (one fewer degree than number of bins),^[This is
based on a normal approximation, using the fact that if $U_1, \ldots,
...
percent when converted back to the probabilty scale).  The parameter
...
# Hiearchical Model with Item Difficulty
...
init_fun_hier <- function(chain_id) {
  ...
  list(pi = pi_init, mu_theta = mu_theta_init,
       ...
       sigma_beta = sigma_beta_init, beta = beta_init)
}
```

and then a call to sampling,
...
fit_hier <- sampling(model_hier, data = list(I = I, J = 5, y = y),
                     init = init_fun_hier,
		     seed = 1234,
                     iter = M_hier / 2, chains = 2, refresh = 1,
		     control = list(adapt_delta = 0.95, stepsize = 0.01),
                     open_progress = FALSE)
```

Let's see the quantities of interest,
...
Text Copyright (2019) Bob Carpenter.  Text released under the CC-BY NC 4.0 license.

Code Copyright (2019) Trustees of Columbia University.  Code is
