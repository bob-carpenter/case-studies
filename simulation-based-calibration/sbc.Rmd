...
\end{array}
$$
Then the value of $\textrm{I_lt_sim}$ will be the following table,
$$
\begin{array}{r|rr}
  & \mu^{(m)} < \mu^{\textrm{sim}} &
    \sigma^{(m)} < \sigma^{\textrm{sim}}
\\[4pt]
m & \textrm{I_lt_sim}_1 & \textrm{I_lt_sim}_2
\\[4pt] \hline
1 & 0 & 0 \\
2 & 1 & 1 \\
3 & 1 & 0 \\
...
These totals are just the sums of the columns.  To get final ranks
numbered from one, we will add one to these sums (which may be zero).
...
num_monitored_params <-
  function(model, data = list()) {
...
would expect.  Here are the histog
...
branch before I wrote this case study.
